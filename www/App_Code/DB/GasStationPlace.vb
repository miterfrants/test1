...
Namespace DB
    Partial Public Class GasStationPlace
        ...
        Shared Function GetTotalCount() As Int32
            Return UW.SQL.GetSingleIntWithDefault("Select count(*) from " & DB.GasStationPlace.BaseTableName_Const)
        End Function

        Shared Function GetSum(ByVal FieldNames As String) As Int32
            Return UW.SQL.GetSingleIntWithDefault("Select sum(" & FieldNames & ") from " & DB.GasStationPlace.BaseTableName_Const)
        End Function
        ...
        Shared Function GetAllDataFromBaseTableWithCache(Optional ByVal OrderFields As String = "_Default", Optional ByVal IsCopy As Boolean = True) As DataTable
            Dim CacheName As String = "GetAllDataFromBaseTableWithCache_GasStationPlace_" & OrderFields
        ...
        Shared Function GetGasStationPlaceFromCachedDT(ByVal Key As String) As DB.GasStationPlace
            Dim DT As DataTable = GetAllDataFromBaseTableWithCache()
            Dim Rows As DataRow() = DT.Select(IdField_Const & " = '" & Key.Replace("'", "''") & "'")
            If Rows.Length > 0 Then
                Return New GasStationPlace(Rows(0))
            Else
                Return Nothing
            End If
        End Function
        ...
        Shared Function getPlaceInRadius(ByVal radius As Decimal, ByVal lat As Decimal, ByVal lng As Decimal) As List(Of GasStationPlace)
            Dim oP As New DB.GasStationPlace()
            oP.AddCriteria("latitude <> 0 and geo_location.STDistance(geography::Point(" & lat & "," & lng & ",4326)) <1000")
            Dim list As New List(Of DB.GasStationPlace)
            Dim dt As DataTable = oP.GetQueryDT()
            If dt IsNot Nothing Then
                For i As Int32 = 0 To dt.Rows.Count - 1
                    Dim itemP As New DB.GasStationPlace(dt.Rows(i))
                    list.Add(itemP)
                Next
            End If
            Return list
