...
Namespace DB
    Partial Public Class MobileReportPullDownCount
        ...
        Shared Function GetTotalCount() As Int32
            Return UW.SQL.GetSingleIntWithDefault("Select count(*) from " & DB.MobileReportPullDownCount.BaseTableName_Const)
        End Function

        Shared Function GetSum(ByVal FieldNames As String) As Int32
            Return UW.SQL.GetSingleIntWithDefault("Select sum(" & FieldNames & ") from " & DB.MobileReportPullDownCount.BaseTableName_Const)
        End Function
        ...
        Shared Function GetAllDataFromBaseTableWithCache(Optional ByVal OrderFields As String = "_Default", Optional ByVal IsCopy As Boolean = True) As DataTable
            Dim CacheName As String = "GetAllDataFromBaseTableWithCache_MobileReportPullDownCount_" & OrderFields
        ...
        Shared Function GetMobileReportPullDownCountFromCachedDT(ByVal Key As String) As DB.MobileReportPullDownCount
            Dim DT As DataTable = GetAllDataFromBaseTableWithCache()
            Dim Rows As DataRow() = DT.Select(IdField_Const & " = '" & Key.Replace("'", "''") & "'")
            If Rows.Length > 0 Then
                Return New MobileReportPullDownCount(Rows(0))
            Else
                Return Nothing
            End If
        End Function
