...
Namespace DB
    Partial Public Class RentalPlace
        ...
        Shared Function GetTotalCount() As Int32
            Return UW.SQL.GetSingleIntWithDefault("Select count(*) from " & DB.RentalPlace.BaseTableName_Const)
        End Function

        Shared Function GetSum(ByVal FieldNames As String) As Int32
            Return UW.SQL.GetSingleIntWithDefault("Select sum(" & FieldNames & ") from " & DB.RentalPlace.BaseTableName_Const)
        End Function
        ...
        Shared Function GetAllDataFromBaseTableWithCache(Optional ByVal OrderFields As String = "_Default", Optional ByVal IsCopy As Boolean = True) As DataTable
            Dim CacheName As String = "GetAllDataFromBaseTableWithCache_RentalPlace_" & OrderFields
        ...
        Shared Function getPlaceInRadius(ByVal radius As Decimal, ByVal lat As Decimal, ByVal lng As Decimal) As List(Of RentalPlace)
            Dim oAP As New DB.RentalPlace()
            oAP.AddCriteria("latitude <> 0 and geo_location.STDistance(geography::Point(" & lat & "," & lng & ",4326)) <1000")
            Dim list As New List(Of DB.RentalPlace)
            Dim dt As DataTable = oAP.GetQueryDT()
            If dt IsNot Nothing Then
                For i As Int32 = 0 To dt.Rows.Count - 1
                    Dim itemAp As New DB.RentalPlace(dt.Rows(i))
                    list.Add(itemAp)
                Next
            End If
            Return list
