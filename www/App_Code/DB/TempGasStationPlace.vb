...
Namespace DB
    Partial Public Class TempGasStationPlace
        ...
        Shared Function GetTotalCount() As Int32
            Return UW.SQL.GetSingleIntWithDefault("Select count(*) from " & DB.TempGasStationPlace.BaseTableName_Const)
        End Function

        Shared Function GetSum(ByVal FieldNames As String) As Int32
            Return UW.SQL.GetSingleIntWithDefault("Select sum(" & FieldNames & ") from " & DB.TempGasStationPlace.BaseTableName_Const)
        End Function
        ...
        Shared Function GetAllDataFromBaseTableWithCache(Optional ByVal OrderFields As String = "_Default", Optional ByVal IsCopy As Boolean = True) As DataTable
            Dim CacheName As String = "GetAllDataFromBaseTableWithCache_TempGasStationPlace_" & OrderFields
        ...
        Shared Function GetTempGasStationPlaceFromCachedDT(ByVal Key As String) As DB.TempGasStationPlace
            Dim DT As DataTable = GetAllDataFromBaseTableWithCache()
            Dim Rows As DataRow() = DT.Select(IdField_Const & " = '" & Key.Replace("'", "''") & "'")
            If Rows.Length > 0 Then
                Return New TempGasStationPlace(Rows(0))
            Else
                Return Nothing
            End If
        End Function
