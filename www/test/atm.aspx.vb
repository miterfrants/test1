...
Partial Class test_atm
    ...
    Sub getFamily()
        ...
        For i As Int32 = 0 To options.Count - 1
            ...
            Dim nodes As HtmlAgilityPack.HtmlNodeCollection = iniPageHtml.DocumentNode.SelectNodes("//div[@class='nblist']/a")
            For j As Int32 = 1 To nodes.Count - 3
                Dim PageString As String = UW.WU.getRemotePage(url.Replace("{page}", j + 1).Replace("{city}", options.Item(i).Attributes("value").Value))
    ...
    Sub updateDiffGeometry(ByVal enType As DB.AtmPlace.EN.type)
        ...
        If dtDelete IsNot Nothing Then
            For i As Int32 = 0 To dtDelete.Rows.Count - 1
                ...
                If oAtmPlace.deleted_count = 2 Then
                    oAtmPlace.Delete()
                Else
